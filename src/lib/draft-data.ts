...
export async function getFleaflickerDraftData(leagueId: string, formatParam?: string, keepersParam?: string) {
    ...
    const prospects = await db.select({ full_name: prospectData.full_name, nfl_team: prospectData.nfl_team, zap_score: prospectData.zap_score, zap_category: prospectData.zap_category, is_year_2: prospectData.is_year_2, statistical_comparables: prospectData.statistical_comparables, analysis_text: prospectData.analysis_text, rookie_rank: prospectData.rookie_rank, rookie_pos_rank: prospectData.rookie_pos_rank, rookie_tier: prospectData.rookie_tier, target_fade: prospectData.target_fade, ai_confidence: prospectData.ai_confidence, ai_summary: prospectData.ai_summary, ai_bull_case: prospectData.ai_bull_case, ai_bear_case: prospectData.ai_bear_case, ai_comps: prospectData.ai_comps }).from(prospectData).where(sql`${prospectData.draft_year} >= ${currentYear - 1}`);
    ...
    const allPlayersWithZap = allPlayersData.map(p => {
        ...
        if (!zap) return { ...p, zap_score: null, zap_category: null, zap_stale: false, zap_comps: null, zap_analysis: null, zap_ai: null, zap_nfl_team: null, rookie_rank: null, rookie_pos_rank: null, rookie_tier: null, target_fade: null, writeups: null };
        ...
        return { ...p, zap_score: score, zap_category: zap.zap_category || null, zap_stale: stale, zap_comps: zap.statistical_comparables || null, zap_analysis: zap.analysis_text || null, zap_ai, zap_nfl_team: zap.nfl_team || null, rookie_rank: zap.rookie_rank, rookie_pos_rank: zap.rookie_pos_rank, rookie_tier: zap.rookie_tier, target_fade: zap.target_fade || null, writeups: null as any };
    });
...
export async function getSleeperDraftData(leagueId: string, formatParam?: string, keepersParam?: string) {
    ...
    const addZap = <T extends { full_name: string; years_exp?: number | null }>(p: T) => {
        ...
        const zap_ai = zap?.ai_summary ? { confidence: zap.ai_confidence, summary: zap.ai_summary, bull_case: zap.ai_bull_case, bear_case: zap.ai_bear_case, comps: zap.ai_comps } : null;
        if (!zap) return { ...p, zap_score: null, zap_category: null, zap_stale: false, zap_comps: null, zap_analysis: null, zap_ai: null, zap_nfl_team: null, rookie_rank: null, rookie_pos_rank: null, rookie_tier: null, target_fade: null, writeups: w };
        const score = parseFloat(zap.zap_score || '0'); const isRookie = p.years_exp === 0; const stale = !isRookie && !zap.is_year_2;
        return { ...p, zap_score: score, zap_category: zap.zap_category || null, zap_stale: stale, zap_comps: zap.statistical_comparables || null, zap_analysis: zap.analysis_text || null, zap_ai, zap_nfl_team: zap.nfl_team || null, rookie_rank: zap.rookie_rank, rookie_pos_rank: zap.rookie_pos_rank, rookie_tier: zap.rookie_tier, target_fade: zap.target_fade || null, writeups: w };
    };

...
export default function Home() {
  ...
  // MyFFPC leagues
  const [myffpcLeagues, setMyffpcLeagues] = useState<{ league_id: string; name: string | null; total_rosters: number | null }[]>([]);
  useEffect(() => {
    fetch('/api/myffpc?list=true')
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data?.leagues) setMyffpcLeagues(data.leagues); })
      .catch(() => {});
  }, []);

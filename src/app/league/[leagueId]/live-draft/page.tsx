...
export default async function SleeperLiveDraftPage({ params, searchParams }: { params: Promise<{ leagueId: string }>; searchParams: Promise<{ format?: string; keepers?: string }> }) {
    const { leagueId } = await params;
    const { format, keepers } = await searchParams;
    const data = await getSleeperDraftData(leagueId, format, keepers);

    return <MockDraftClient leagueId={leagueId} {...data} platform="sleeper" mode="live" />;
}
